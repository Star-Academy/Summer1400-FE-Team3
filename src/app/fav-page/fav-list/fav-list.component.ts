...
export class FavListComponent implements OnInit {
  ...
  @Input() set searchInput(value: string) {
    ...
    if (this._searchInput !== '') {
      this.favSongs = this.favSongs.filter((song) => {
        return (
          song.artist.includes(this._searchInput) ||
          song.name.includes(this._searchInput) ||
          song.lyrics.includes(this._searchInput)
        );
      });
      if (this.favSongs.length === 0) {
        this.hiddenNoResult = false;
